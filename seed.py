...
def init_db():
    ...
    with app.app_context():
        ...
        if not admin:
            ...
            admin = User(username='admin', role='admin', full_name='Hệ thống Admin', email='admin@petshop.com')
            admin.set_password('123456')
            db.session.add(admin)
        
        # 2. Staff
        if not User.query.filter_by(username='receptionist1').first():
            receptionist = User(username='receptionist1', role='receptionist', full_name='Nguyễn Lễ Tân', email='letan@petshop.com')
            receptionist.set_password('123456')
            db.session.add(receptionist)

        if not User.query.filter_by(username='vet1').first():
            vet = User(username='vet1', role='veterinarian', full_name='BS. Trần Thú Y', email='thuy@petshop.com')
            vet.set_password('123456')
            db.session.add(vet)
